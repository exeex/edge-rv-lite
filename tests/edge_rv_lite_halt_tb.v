...
module edge_rv_lite_halt_tb;
  ...
  edge_rv_lite_core dut(
    ...
    .dmem_resp_valid(dmem_req_valid), .dmem_resp_error(1'b0),
  ...
  task run_case;
    ...
    begin
      ...
      if (!halted || illegal != expected_illegal)
        $fatal(1, "case %0d terminal state mismatch", selected_case);
      halted_instret = instret_count;
      halted_pc = dut.ex_pc;
      repeat (10) begin
        @(posedge clk);
        if (instret_count != halted_instret || dut.gpr[5] != 0 ||
            dut.ex_pc != halted_pc)
          $fatal(1, "case %0d architectural state changed after halt",
                 selected_case);
      end
      if (halted_instret != expected_instret || dmem_requests != 0)
  ...
  initial begin
    run_case(EBREAK_CASE, 64'd1, 1'b0);
    run_case(ILLEGAL_CASE, 64'd0, 1'b1);
    $display("TEST PASS: terminal halt flushes younger work and stays quiescent");
    $finish;
  end
endmodule
