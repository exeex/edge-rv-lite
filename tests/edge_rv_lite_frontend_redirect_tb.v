...
module edge_rv_lite_frontend_redirect_tb;
  ...
  wire issue_valid; reg issue_ready = 1;
  wire [39:0] issue_pc; wire [31:0] issue_inst; wire issue_error;
  ...
  initial begin
    ...
    wait (issue_valid);
    if (issue_pc != 0) begin $display("bad reset PC"); $finish; end
    // Model EX resolving a taken branch while a sequential fetch is pending.
    @(posedge clk); issue_ready <= 0;
    wait (imem_req_valid); @(posedge clk);
    ...
    @(posedge clk); imem_resp_valid <= 0; issue_ready <= 1;
    wait (imem_req_valid);
    ...
    @(posedge clk); imem_resp_valid <= 0;
    wait (issue_valid);
    if ((issue_pc != 40'h80) || (issue_inst == 32'hdead_beef)) begin
